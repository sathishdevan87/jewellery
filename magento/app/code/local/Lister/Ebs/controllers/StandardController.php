...
/**
 * Ebs Standard Front Controller
 *
 * @category   Mage
 * @package    Lister_Ebs
 * @name       Lister_Ebs_StandardController
 * @author     Magento Core Team <core@magentocommerce.com>
*/

require('Rc43.php');

class Lister_Ebs_StandardController extends Mage_Core_Controller_Front_Action
{
    /**
     * Order instance
     */
    ...
    {
        return Mage::getSingleton('ebs/config')->getDebug();
    }
    ...
    /**
     * When a customer chooses Ebs on Checkout/Payment page
     *
     */
...
$this->getResponse()->setBody($this->getLayout()->createBlock('ebs/standard_redirect')->toHtml());
    ...
    public function  successAction()
    {
     ...
     if(isset($_GET['DR'])) {
	     ...
	     $secret_key = Mage::getSingleton('ebs/config')->getSecretKey(); // Your Secret Key
	 ...
	 foreach($QueryString as $param){
       ...
       {
       	$session = Mage::getSingleton('checkout/session');
        $session->setQuoteId($session->getEbsStandardQuoteId());
        $session->unsEbsStandardQuoteId();
        ...
        $order->addStatusToHistory(
            $order->getStatus(),
            Mage::helper('ebs')->__('Customer successfully returned from Ebs')
        );
...
$order->sendNewOrderEmail();
       ...
       {
       	$this->_redirect('ebs/standard/failure');
       }
    }
    ...
    /**
     *  Failure response from Ebs
     *
     *  @return	  void
     */
    public function failureAction ()
    {
        $errorMsg = Mage::helper('ebs')->__('There was an error occurred during paying process.');
